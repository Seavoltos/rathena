...
void run_tomb(map_session_data* sd, npc_data* nd)
{
	...
	clif_scriptmes(*sd, nd->id, msg_txt(sd, 2550));
		...
		*/
		if(md->state.boss){
			...
			if(1 == mapreg_readreg(reference_uid( add_str( mapregname.c_str() ), 0 ))){ //1 = dead - 2 = alive
				time_t timer = static_cast<time_t>(mapreg_readreg(reference_uid(add_str(mapregname.c_str()),3)));
				time_t now = time(NULL);

				int difftime = mob->delay1; //Base respawn time
				...
				if(difftime < 0){ //mvp is still dead
					if (battle_config.mvp_tomb_enabled && map_getmapflag(md->bl.m, MF_NOTOMB) != 1){ //is tomb enabled ?
						...
						md->bl.x = mapreg_readreg(reference_uid(add_str(mapregname.c_str()),1));
						md->bl.y = mapreg_readreg(reference_uid(add_str(mapregname.c_str()),2));
